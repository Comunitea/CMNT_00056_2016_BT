    ...
    def _create_invoice_from_picking(self, picking, vals):
        ...
        if picking and picking.sale_id and inv_type == "out_invoice":
            ...
            if (
                ...
                and sale_order.payment_mode_id.payment_order_type == "debit"
            ):
                invoice = self.env["account.invoice"].browse(invoice_id)
                mandate_obj = self.env["account.banking.mandate"]
                mandates = mandate_obj.search(
                    [
                        ("partner_bank_id", "in", invoice.partner_id.bank_ids.ids),
                        ("state", "=", "valid"),
                    ]
                )
                mandate_sel = False
                ...
                if mandate_sel:
                    ...
                    invoice.partner_bank_id = invoice.partner_id.bank_ids[0].id
        return invoice_id
