...
'child_of', [partner_id])]""",
    ...
    def onchange_partner_id(
        ...
        if partner_id:
            ...
            if (
                type == "out_invoice"
                and partner.customer_payment_mode
                and partner.customer_payment_mode.payment_order_type == "debit"
                ...
                if mandate_sel:
                    res["value"].update(
                        {
                            "mandate_id": mandate_sel.id,
                            "partner_bank_id": mandate_sel.partner_bank_id.id,
                        }
                    )
                else:
                    res["value"].update({"partner_bank_id": partner.bank_ids[0].id})
            elif (
                type == "out_invoice"
                and res.get("value")
                and res["value"].get("partner_bank_id")
            ):
                account_id = self.env["res.partner.bank"].browse(
                    res["value"]["partner_bank_id"]
                )
                if account_id.state == "bank":
                    del res["value"]["partner_bank_id"]
        return res
    ...
    def create(self, vals):
        if (
            ...
            and vals.get("payment_mode_id", False)
            and not vals.get("sdd_mandate_id", False)
        ):
            pmode = self.env["payment.mode"].browse(vals["payment_mode_id"])
    ...
    def write(self, vals):
        if vals.get("partner_bank_id", False) and not vals.get("sdd_mandate_id", False):
            ...
            if pmode_id:
                pmode = self.env["payment.mode"].browse(pmode_id)
                if pmode.payment_order_type == "debit":
                    mandate_obj = self.env["account.banking.mandate"]
                    mandates = mandate_obj.search(
                        [
