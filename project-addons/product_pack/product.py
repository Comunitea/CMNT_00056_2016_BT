...
class product_pack(orm.Model):
    ...
    def update_pack_products(self, cr, uid, ids, context=None):
        for update_line in self.browse(cr, uid, ids):
            ...
            for pack_line in update_line.parent_product_id.pack_line_ids:
                cost_price += pack_line.product_id.standard_price * pack_line.quantity
    ...
    def write(self, cr, uid, ids, vals, context):
        res = super(product_pack, self).write(cr, uid, ids, vals, context=context)
...
class product_product(orm.Model):
            ...
            if product.pack_line_ids:
                ...
                for subproduct in product.pack_line_ids:
                    ...
                    subproduct_stock_next = self._product_available(
                        cr, uid, [subproduct.product_id.id], field_names, arg, context
                    )[subproduct.product_id.id]["qty_available"]
    ...
    _defaults = {"pack_fixed_price": True}
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        if "standard_price" in vals:
            for prod in self.browse(cr, uid, ids, context=context):
                if vals["standard_price"] != prod.standard_price:
                    pline_ids = self.pool.get("product.pack.line").search(
        ...
        if pack_lines_to_update:
            self.pool.get("product.pack.line").update_pack_products(
                cr, uid, pack_lines_to_update, context=context
            )
        return res
