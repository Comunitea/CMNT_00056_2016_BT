...
class StockPicking(models.Model):
    ...
    @api.multi
    def send_vpe(self, carrier_api):
        """
        Send shipments out to vpe
        :param api: obj
        ...
        for picking in self:
            ...
            if not service:
                raise exceptions.Warning(
                    _("Api service error"),
                    _("Select a service or default service in vpe API"),
                )
            ...
            reference = carrier_api.send_picking_vpe(data)
            picking.write(
                {
            ...
            labels += picking.print_labels_vpe(carrier_api)
        return references, labels

    @api.multi
    def print_labels_vpe(self, carrier_api):
        ...
        for picking in self:
            ...
            if not reference:
                logger.error("Picking %s has not been sent by vpe." % (picking.name))
                continue

            vals = {"picking_ref": reference}

            label = carrier_api.label_vpe(vals)
            if not label:
                logger.error(
                    "Label for picking %s is not available from vpe." % picking.name
                )
                continue
            with tempfile.NamedTemporaryFile(
                prefix="%s-vpe-%s-" % (dbname, reference), suffix=".pdf", delete=False
            ) as temp:
                temp.write(decodestring(label))  # vpe PDF file
