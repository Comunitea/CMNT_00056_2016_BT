...
class StockPicking(models.Model):
    ...
    @api.multi
    def correosexpress_picking_data(self, api, service, price=None, weight=False):
        """
        ...
        data = {
            ...
            "dirRte": unaccent(remitente_partner.street)
            + " "
            ...
            "dirDest": unaccent(self.partner_id.street)
            + " "
            ...
            for picking in self:
                ...
                if not service:
                    raise exceptions.Warning(
                        _("Api service error"),
                        _(
                            "Select a service or default service in º\
Correos express API"
                        ),
                    )
                ...
                if picking.cash_on_delivery:
                    ...
                    if not price:
                        raise exceptions.Warning(
                            _("Picking error"),
                            _(
                                'Shipment "%s" not have price and send \
cashondelivery'
                            )
                            % picking.name,
                        )
                ...
                if reference:
                    ...
                    logger.error("Not send shipment %s." % (picking.name))
                if returned_labels:
                    x = 0
                    for label in returned_labels:
                        with tempfile.NamedTemporaryFile(
                            prefix="%s-correosexpress-%s-%s" % (dbname, reference, x),
                            suffix=".pdf",
                            delete=False,
                        ) as temp:
                            temp.write(label)
