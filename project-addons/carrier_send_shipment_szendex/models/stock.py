        ...
        for picking in self:
            ...
            if not service:
                raise exceptions.Warning(
                    _("Api service error"),
                    _("Select a service or default service in szendex API"),
                )
    ...
    def print_labels_szendex(self, carrier_api, guid=False):
        ...
        for picking in self:
            if not picking.carrier_tracking_ref:
                logger.error(
                    "Picking %s has not been sent by szendex." % (picking.name)
                )
                continue

            vals = {"reference": picking.carrier_tracking_ref, "guid": guid}
            ...
            if not label:
                logger.error(
                    "Label for picking %s is not available from szendex." % picking.name
                )
                continue
