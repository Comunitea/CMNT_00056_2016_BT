...
class StockPicking(models.Model):
    ...
    def asm_picking_data(self, api, service, price=None, weight=False):
        ...
        data["remite_provincia"] = (
            remitente_partner.state_id
            and unaccent(remitente_partner.state_id.name)
            or ""
        )
        data["remite_pais"] = (
            remitente_partner.country_id and remitente_partner.country_id.code
        )
        ...
        data["destinatario_provincia"] = (
            self.partner_id.state_id and unaccent(self.partner_id.state_id.name) or ""
        )
        data["destinatario_pais"] = (
            self.partner_id.country_id and self.partner_id.country_id.code or ""
        )
        data["destinatario_cp"] = self.partner_id.zip
        data["destinatario_telefono"] = (
            self.partner_id.phone and unaccent(self.partner_id.phone) or ""
        )
        data["destinatario_movil"] = (
            self.partner_id.mobile and unaccent(self.partner_id.mobile) or ""
        )
        data["destinatario_email"] = (
            self.partner_id.email
            and unaccent(self.partner_id.email)
            or self.partner_id.commercial_partner_id.email
            and unaccent(self.partner_id.commercial_partner_id.email)
            or ""
        )
        data["destinatario_observaciones"] = unaccent(notes)
        data["destinatario_att"] = unaccent(
            remitente_partner.name if remitente_partner.name else self.partner_id.name
        )
            ...
            for picking in self:
                ...
                if not service:
                    raise exceptions.Warning(
                        _("Api service error"),
                        _("Select a service or default service in ASM API"),
                    )

                if not picking.partner_id.country_id:
                    raise exceptions.Warning(
                        _("Partner error"),
                        _('Add country in shipment "%s" partner')
                ...
                logger.info(
                    "Sended picking: %s received: reference: %s label: %s error: %s"
                    % (picking.name, reference, label and "1" or "0", error)
                )
                if reference:
                    picking.write(
                        {
                    ...
                    logger.info("Send shipment %s" % (picking.name))
                    references.append(picking.name)
                else:
                    logger.error("Not send shipment %s. %s" % (picking.name, error))
                if label:
        ...
        with Picking(api.username, api.debug) as picking_api:
            for picking in self:
                if not picking.carrier_tracking_ref:
                    logger.error(
                        "Picking %s has not been sent by ASM." % (picking.name)
                    )
                    continue
                ...
                if not label:
                    logger.error(
                        "Label for picking %s is not available from ASM." % picking.name
                    )
                    continue
                with tempfile.NamedTemporaryFile(
                    prefix="%s-asm-%s-" % (dbname, reference),
