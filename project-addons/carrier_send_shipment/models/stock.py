...
class StockPicking(models.Model):
    ...
    ship_return = fields.Boolean(
        "Return",
        related="sale_id.ship_return",
        readonly=True,
        help="Active return when send API shipment",
    )

    carrier_service = fields.Many2one("carrier.api.service", "Carrier API Service")
    carrier_delivery = fields.Boolean(
        "Delivered", readonly=True, help="The package has been delivered", copy=False
    )
    carrier_printed = fields.Boolean(
        "Printed", readonly=True, help="Picking is already printed", copy=False
    )
    ...
    weight_edit = fields.Float(
        "Weight", compute="compute_weight", store=True, readonly=False
    )
    weight_net_edit = fields.Float(
        "Net weight", compute="compute_weight", store=True, readonly=False
    )
    ...
    def send_shipment_api(self):
        ...
        if not self.carrier_id:
            raise exceptions.Warning(
                _("No carrier"), _('Shipment "%s" not have carrier') % self.name
            )

        api = self.env["carrier.api"].search(
            [("carriers", "in", [self.carrier_id.id])], limit=1
        )
        if not api:
            raise exceptions.Warning(
                _("No api"), _('Carrier "%s" not have API') % self.carrier.name
            )

        if (
            not self.partner_id.street
            or not self.partner_id.zip
            or not self.partner_id.city
            or not self.partner_id.country_id
        ):
            raise exceptions.Warning(
                _("Address error"),
                _(
                    'Shipment "%s" not have address details: street, zip, city or country.'
                )
                % self.name,
            )
    ...
    def barcode_send_shipment(self):
        logger.info(
            u"ASM: llamada a servidor de envío pick: %s con user: %s"
            % (self[0].name, self.env.user.name)
        )
