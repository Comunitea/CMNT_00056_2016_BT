...
class TestOnchangeProductId(TransactionCase):
    ...
    def setUp(self):
        ...
        self.product = self.env["product.product"].create(
            {"name": "Test Product", "lst_price": 10.00}
        )
        version_dict1 = {
            "name": "Test version 1",
            "items_id": [
                (0, 0, {"name": "Test item default", "base": 1, "price_discount": 0.1})
            ],
        }
        version_dict2 = {
            "name": "Test version 2",
            "items_id": [
                (0, 0, {"name": "Test item default", "base": 1, "price_discount": 0.2})
            ],
        }
        ...
        self.partner = self.env["res.partner"].create(
            {"name": "Test partner", "property_product_pricelist": self.plist1.id}
        )
        self.invoice_line = self.env["account.invoice.line"].create(
            {"name": "Test line", "quantity": 1.0, "price_unit": 1.0}
        )
    ...
    def test_onchange_product_id_pricelist(self):
        ...
        self.assertLessEqual(
            abs(exp_value - res["value"]["price_unit"]),
            0.0001,
    ...
    def test_onchange_product_id_invoice_pricelist(self):
        ...
        res = self.invoice_line.with_context(
            {"pricelist_id": self.plist2.id}
        ).product_id_change(
            self.product.id,
            self.product.uom_id.id,
            qty=1.0,
