...
class AccountInvoice(models.Model):
    ...
    def action_move_create(self):
        ...
        for invoice in self:
            if (
                not self.sale_store_id
                or not self.sale_store_id.not_create_invoice_moves
            ):
            ...
            for inv in self - super_invoices:
                if not inv.invoice_number:
                    ...
                    if sequence:
                        ...
                        number = False
                    inv.write({"number": number, "invoice_number": number})
                else:
                    inv.write({"number": inv.invoice_number})
