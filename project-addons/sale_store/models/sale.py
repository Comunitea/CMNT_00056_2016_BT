...
class SaleStore(models.Model):
    ...
    partner_id = fields.Many2one(
        "res.partner", compute="_compute_partner_fields", readonly=True
    )
    default_account_id = fields.Many2one(
        "account.account", "Default account for ecommerce customers importations"
    )
...
class SaleStoreConfig(models.Model):
    ...
    state_id = fields.Many2one(
        "res.country.state", related="partner.state_id", readonly=True
    )
...
class SaleOrder(models.Model):
    ...
    sale_store_id = fields.Many2one(
        "sale.store", "Store", readonly=True, states={"draft": [("readonly", False)]}
    )
    ...
    def onchange_partner_id(self, part):
        ...
        if store:
            ...
            payment_term = partner.get_store_value(store, "account.payment.term")
            if payment_mode_id:
    ...
    def onchange_sale_store_id(self):
        if self.partner_id and self.sale_store_id:
            ...
            pricelist_id = self.partner_id.get_store_value(
                self.sale_store_id, "product.pricelist"
            )
